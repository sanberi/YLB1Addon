...
Public NotInheritable Class TI_Z0006
    Inherits FormBase
    ...
    Private Sub TI_Z0006_ItemEvent(FormUID As String, ByRef pVal As ItemEvent, ByRef BubbleEvent As Boolean) Handles Me.ItemEvent
        Select Case pVal.EventType
                ...
                If pVal.BeforeAction Then
                    ...
                    Dim lsSQL As String = "select T10.U_TempPath,T11.U_Template,T11.U_PrintName,T11.U_PageSize,T11.U_TempName " &
                                            "   from [@ti_z0010] T10 inner join [@ti_z0011] T11 On t10.Code=t11.code  " &
                                              " where T10.code ='OWTR' and isnull(T11.U_TempName,'') <>'' "
                    ...
                    Next
                    loCmb_Chooselist.Select("借货单", BoSearchKey.psk_ByValue)
                End If
            Case BoEventTypes.et_ITEM_PRESSED
                ...
                If Not pVal.Before_Action And pVal.ItemUID = "Export" Then
                    ...
                    '    MyApplication.MessageBox("只有在确认模式下才能打印!", 1, "确定", "取消")
                    '    Return
                    'End If
                    ...
                    Dim lsSQL As String = "select T10.U_TempPath,T11.U_Template,T11.U_PrintName,T11.U_PageSize " &
                                              "   from [@ti_z0010] T10 inner join [@ti_z0011] T11 On t10.Code=t11.code  " &
                                                " where T10.code ='OWTR' and T11.U_tempname='" & lsTempName & "'"
                    ioTempSql.ExecuteQuery(lsSQL)
                    ...
                    If lsTempName.Contains("草稿") Then
                        liDocEntry = MyForm.DataSources.DBDataSources.Item(0).GetValue("DocEntry", 0)
                    Else
                        liDocEntry = MyForm.Items.Item("11").Specific.value
                    End If
