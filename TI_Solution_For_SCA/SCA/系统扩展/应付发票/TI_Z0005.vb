                ...
                If pVal.BeforeAction Then
                    ...
                    Dim lsSQL As String = "select T10.U_TempPath,T11.U_Template,T11.U_PrintName,T11.U_PageSize,T11.U_TempName " &
                                            "   from [@ti_z0010] T10 inner join [@ti_z0011] T11 On t10.Code=t11.code  " &
                                              " where T10.code ='OPCH' and isnull(T11.U_TempName,'') <>'' "
                    ...
                    Next
                    loCmb_Chooselist.Select("应付发票草稿", BoSearchKey.psk_ByValue)
                End If
            Case BoEventTypes.et_ITEM_PRESSED
                ...
                If Not pVal.Before_Action And pVal.ItemUID = "Export" Then
                    ...
                    Dim lsSQL As String = "select T10.U_TempPath,T11.U_Template,T11.U_PrintName,T11.U_PageSize " &
                                              "   from [@ti_z0010] T10 inner join [@ti_z0011] T11 On t10.Code=t11.code  " &
                                                " where T10.code ='OPCH' and T11.U_tempname='" & lsTempName & "'"
                    ioTempSql.ExecuteQuery(lsSQL)
                    ...
                    lsRepeatSQL = "Select  U_RepeatCopy,U_RepeatUser  from [@TI_Z0011] T10 where T10.Code='OPCH' and T10.U_TempName ='" & lsTempName.Trim & "'"
                    ioTempSql.ExecuteQuery(lsRepeatSQL)
                    If ioTempSql.Rows.Count > 0 Then
                        lsRepeatUser = ioTempSql.GetValue("U_RepeatUser", 0)    '可重复打印人员
                        Integer.TryParse(ioTempSql.GetValue("U_RepeatCopy", 0), liRepeatCopy)   '可重复打印份数
                    End If
                    If lsRepeatUser.Trim <> MyApplication.Company.UserName Then
                        lsSQLPrinted = "select isnull(count(*),0) as 'PrintedCopy',max(PrintUser) as 'PrintUser',max(PrintDate) as 'PrintDate' from PrintLog where TransType  ='112' and Objtype ='18' and TemplateName ='" & lsTempName.Trim & "' and DocEntry ='" & liDocEntry.ToString() & "' "
                        ioTempSql.ExecuteQuery(lsSQLPrinted)
                        If ioTempSql.Rows.Count > 0 Then
                            Integer.TryParse(ioTempSql.GetValue("PrintedCopy", 0), liPrintedCopy)
                            lsPrintedUser = ioTempSql.GetValue("PrintUser", 0)
                            lsPrintedDatetime = ioTempSql.GetValue("PrintDate", 0)
                            If liPrintedCopy <> 0 Then
                                MyApplication.MessageBox("该打印模板已经在" + lsPrintedDatetime + "被用户" + lsPrintedUser.Trim + "打印过，不能再次打印，要重复打印，请提交重复打印申请！")
                                Return
                            End If
                        End If
                    Else
                        lsSQLPrinted = "select isnull(count(*),0) as 'PrintedCopy' from PrintLog where TransType  ='112' and Objtype ='18' and TemplateName ='" & lsTempName.Trim & "' and DocEntry ='" & liDocEntry.ToString() & "' and Printuser='" + lsRepeatUser.Trim() + "' "
                        ioTempSql.ExecuteQuery(lsSQLPrinted)
                        If ioTempSql.Rows.Count > 0 Then
                            Integer.TryParse(ioTempSql.GetValue("PrintedCopy", 0), liPrintedCopy)
                            If liPrintedCopy >= liRepeatCopy Then
                                MyApplication.MessageBox("重复打印次数已经超过可重复次数，不能再次打印，要重复打印，请提交重复打印申请！")
                                Return
                            End If
                        End If
                    End If
